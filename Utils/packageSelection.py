...
class selectPackage():
    ...
    @staticmethod
    def selectNextShortest(unassignedPackages: Packages, currentAddress: str, addressList: dict, truck: Truck, time:timedelta) -> tuple:
        ...
        for package in unassignedPackages.get_packages():#iterate through all provided packages
            packageNote = sn.Actions.translateAction(package.NOTES,package.ID) #Get the translated package note from the package
            avalible, extra = sn.Actions.verifyAvalible(packageNote, truck, time, unassignedPackages) #Check if the package note is avalible.
            ...
            if startAddress.ID > comparePackageAddress.ID: # IF the start addresses ID is greater than the comparisonID use the start addresses' connections list
                ...
                if distance < closest[1] and avalible: #update the closest packge if the distance is closer and the package is avalible
